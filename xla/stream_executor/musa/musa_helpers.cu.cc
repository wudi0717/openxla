...
__global__ void musa_Broadcast_fp32Kernel(float* dst, int dst_stride,
...
void musa_Broadcast_fp32(void* stream, float* dst, int dst_stride, int batches,
                         int src_batches, float* src, int size) {
  int x_blocks = (size + 255) / 256;
  // TODO:
  printf("[ERROR] need to impl musa_helpers 1\n");
  // muLaunchKernel(musa_Broadcast_fp32Kernel,
  //                    dim3(x_blocks, (batches + 3) / 4, src_batches),
  //                    min(256, (int)size), 0, (musaStream_t)stream, dst,
...
void launchInplaceBiasActivation(musaStream_t stream, void* c_data,
  ...
  // TODO:
  printf("[ERROR] need to impl musa_helpers 2\n");
  // muLaunchKernel(kernel, dim3(gx, gy, batch), dim3(bx, by, 1), 0, stream,
...
#define INSTANTIATE_BIAS_ACTIVATION(X, Y)                          \
  template void launchInplaceBiasActivation<X, Y>(                 \
      musaStream_t stream, void* c_data, const void* bias_data,     \
...
INSTANTIATE_BIAS_ACTIVATION(__mt_bfloat16, __mt_bfloat16)
INSTANTIATE_BIAS_ACTIVATION(__mt_bfloat16, float)
