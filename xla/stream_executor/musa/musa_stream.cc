...
absl::StatusOr<musaStream_t> CreateStream(StreamExecutor* executor,
                                         int priority) {
  std::unique_ptr<ActivateContext> activation = executor->Activate();
  musaStream_t stream;
  if (priority == 0) {
...
absl::Status RecordEvent(StreamExecutor* executor, musaEvent_t event,
  ...
  switch (res) {
    ...
    case musaErrorLaunchFailure:
    case musaErrorInvalidResourceHandle:
      return absl::FailedPreconditionError(
          absl::StrFormat("error recording MUSA event on stream %p: %s", stream,
                          ToString(res).c_str()));
    default:
...
absl::Status MusaStream::RecordEvent(Event* event) {
  return stream_executor::gpu::RecordEvent(
      executor_, static_cast<MusaEvent*>(event)->GetHandle(), stream_handle_);
}
...
namespace {
void DestroyStream(StreamExecutor* executor, musaStream_t stream) {
  if (stream == nullptr) {
    return;
  }
...
absl::Status LaunchMusaKernel(
    ...
    unsigned int block_dim_z, unsigned int shared_mem_bytes, musaStream_t stream,
...
absl::Status LaunchMusaKernel(
    StreamExecutor* executor, absl::string_view kernel_name,
    MUfunction function, unsigned int cluster_dim_x,
  ...
  if (cluster_dim_x != 1 || cluster_dim_y != 1 || cluster_dim_z != 1)
    return absl::UnimplementedError("Not implemented for MUSa");
  return LaunchMusaKernel(executor, kernel_name, function, grid_dim_x,
...
absl::Status MusaStream::LaunchKernel(
  ...
  if (cluster_dims.has_value()) {
    return LaunchMusaKernel(
        executor_, name, static_cast<MUfunction>(function), cluster_dims->x,
    ...
    return LaunchMusaKernel(
        executor_, name, static_cast<MUfunction>(function), block_dims.x,
